...
public class ESDK {
  ...
  public DecryptOutput Decrypt(DecryptInput input) {
    software.amazon.cryptography.encryptionsdk.internaldafny.types.DecryptInput dafnyValue =
      ToDafny.DecryptInput(input);
    Result<
      software.amazon.cryptography.encryptionsdk.internaldafny.types.DecryptOutput,
      Error
    > result = this._impl.Decrypt(dafnyValue);
    if (result.is_Failure()) {
      throw ToNative.Error(result.dtor_error());
    }
  ...
  public EncryptOutput Encrypt(EncryptInput input) {
    software.amazon.cryptography.encryptionsdk.internaldafny.types.EncryptInput dafnyValue =
      ToDafny.EncryptInput(input);
    Result<
      software.amazon.cryptography.encryptionsdk.internaldafny.types.EncryptOutput,
      Error
    > result = this._impl.Encrypt(dafnyValue);
    if (result.is_Failure()) {
      throw ToNative.Error(result.dtor_error());
    }
  ...
  public interface Builder {
    Builder AwsEncryptionSdkConfig(
  ...
  static class BuilderImpl implements Builder {
    ...
    public Builder AwsEncryptionSdkConfig(
      AwsEncryptionSdkConfig AwsEncryptionSdkConfig
    ) {
    ...
    public ESDK build() {
      if (Objects.isNull(this.AwsEncryptionSdkConfig())) {
        throw new IllegalArgumentException(
          "Missing value for required field `AwsEncryptionSdkConfig`"
        );
      }
