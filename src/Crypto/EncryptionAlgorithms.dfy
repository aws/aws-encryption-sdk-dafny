...
module {:extern "EncryptionAlgorithms"} EncryptionAlgorithms {
  ...
  datatype EncryptionAlgorithm = EncryptionAlgorithm(alg: Algorithm, keyLen: uint8, tagLen: uint8, ivLen: uint8)
  {
    predicate Valid() {
      match alg
  ...
  const AES_GCM_128 := EncryptionAlgorithm(AES(GCM), 16, AES_TAG_LEN, AES_IV_LEN)
  const AES_GCM_192 := EncryptionAlgorithm(AES(GCM), 24, AES_TAG_LEN, AES_IV_LEN)
  const AES_GCM_256 := EncryptionAlgorithm(AES(GCM), 32, AES_TAG_LEN, AES_IV_LEN)
}
