...
public class __default extends _ExternBase___default {
    public static STL.Result<DafnySequence<Byte>> Encode(DafnySequence<Character> str) {
        ...
        try {
            ...
            return new STL.Result_Success<>(DafnySequence.fromBytes(utf8Bytes));
        } catch (CharacterCodingException e) {
            return new STL.Result_Failure<>(DafnySequence.asString("Input contains invalid Unicode characters"));
        }
    }

    public static STL.Result<DafnySequence<Character>> Decode(DafnySequence<Byte> bytes) {
        CharsetDecoder utf8 = StandardCharsets.UTF_8.newDecoder();
        utf8.onMalformedInput(CodingErrorAction.REPORT);
        utf8.onUnmappableCharacter(CodingErrorAction.REPORT);
        try {
            String decoded = utf8.decode(ByteBuffer.wrap(DafnySequence.toByteArray(bytes))).toString();
