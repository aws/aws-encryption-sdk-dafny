...
module AlgorithmSuite {
  ...
  newtype ID = x | x in VALID_IDS witness 0x0014
  {
    ...
    function method KDFInputKeyLength(): nat
      ensures Suite[this].hkdf == Digests.IDENTITY ==> KDFInputKeyLength() == KeyLength()
    {
      KeyLength()
    }
  ...
  const Suite := map [
    ...
    AES_192_GCM_IV12_TAG16_HKDF_SHA384_ECDSA_P384 := AlgSuite(EncryptionSuites.AES_GCM_192, Digests.HKDF_WITH_SHA_384, Some(S.ECDSA_P384)),
    AES_128_GCM_IV12_TAG16_HKDF_SHA256_ECDSA_P256 := AlgSuite(EncryptionSuites.AES_GCM_128, Digests.HKDF_WITH_SHA_256, Some(S.ECDSA_P256)),
    AES_256_GCM_IV12_TAG16_HKDF_SHA256_SIGNONE    := AlgSuite(EncryptionSuites.AES_GCM_256, Digests.HKDF_WITH_SHA_256, None),
    AES_192_GCM_IV12_TAG16_HKDF_SHA256_SIGNONE    := AlgSuite(EncryptionSuites.AES_GCM_192, Digests.HKDF_WITH_SHA_256, None),
    AES_128_GCM_IV12_TAG16_HKDF_SHA256_SIGNONE    := AlgSuite(EncryptionSuites.AES_GCM_128, Digests.HKDF_WITH_SHA_256, None),
    AES_256_GCM_IV12_TAG16_KDFNONE_SIGNONE        := AlgSuite(EncryptionSuites.AES_GCM_256, Digests.IDENTITY,  None),
    AES_192_GCM_IV12_TAG16_KDFNONE_SIGNONE        := AlgSuite(EncryptionSuites.AES_GCM_192, Digests.IDENTITY,  None),
    AES_128_GCM_IV12_TAG16_KDFNONE_SIGNONE        := AlgSuite(EncryptionSuites.AES_GCM_128, Digests.IDENTITY,  None)
  ]
