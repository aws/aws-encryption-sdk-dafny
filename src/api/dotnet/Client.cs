...
namespace AWSEncryptionSDK
{
    ...
    public class Client {
        ...
        public static ClientEncryptionStream EncryptStream(Stream plaintext, CMM cmm, Dictionary<string, string> encryptionContext = null, ushort? algorithmSuiteID = null, uint? frameLength = null) {
            ...
            // TTODO he way we handle this extern is weird. Determine if there
            // is a clearer way to have an extern implement a Dafny trait, and then
            // be usable in the shim layer.
            InputStreamByteProducer input = new InputStreamByteProducer(plaintext);
            ...
            STL.Result<EncryptionStreams.EncryptionStream> result = ESDKClient.__default.StreamEncrypt(
                    input,
                    cmm,
            ...
            return new ClientEncryptionStream(DafnyFFI.ExtractResult(result));
        }
        ...
        // TODO: Currently just reading all of the input stream into memory, then
        // doing the one shot Decrypt operation
        public static MemoryStream StreamDecrypt(Stream cyphertext, CMM cmm) {
