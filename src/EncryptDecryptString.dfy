...
method EncryptThenDecrypt(arn: string, plaintext_original: string)
{
  ...
  if r != Aws.AWS_OP_SUCCESS { return; }
...
method encrypt_string(key_arn: string,
  ...
  modifies ciphertext
  ensures r == Aws.AWS_OP_SUCCESS ==> ciphertext_len <= ciphertext.Length
  ensures r == Aws.AWS_OP_SUCCESS ==> ciphertext[..ciphertext_len] == Math.Encrypt(plaintext[..plaintext_len])
{
  ...
  var session := new Session.FromCMM(Aws.EncryptMode, cmm);
  cmm.Release();
...
method decrypt_string(key_arn: string,
  ...
  modifies plaintext
  ensures r == Aws.AWS_OP_SUCCESS ==> plaintext_len <= plaintext.Length
  ensures r == Aws.AWS_OP_SUCCESS ==> plaintext[..plaintext_len] == Math.Decrypt(ciphertext[..ciphertext_len])
{
  var kms_keyring := new Aws.KmsKeyring.Build(key_arn);
