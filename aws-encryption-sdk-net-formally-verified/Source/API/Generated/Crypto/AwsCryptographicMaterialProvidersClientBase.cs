...
namespace Aws.Crypto
{
    public abstract class AwsCryptographicMaterialProvidersClientBase : IAwsCryptographicMaterialProviders
    {
        ...
        public Aws.Crypto.ImportPublicRSAKeyOutput ImportPublicRSAKey(Aws.Crypto.ImportRSAKeyInput input)
        {
            input.Validate();
            return _ImportPublicRSAKey(input);
        }

        protected abstract Aws.Crypto.ImportPublicRSAKeyOutput _ImportPublicRSAKey(Aws.Crypto.ImportRSAKeyInput input);

        public Aws.Crypto.ImportPrivateRSAKeyOutput ImportPrivateRSAKey(Aws.Crypto.ImportRSAKeyInput input)
        {
            input.Validate();
            return _ImportPrivateRSAKey(input);
        }

        protected abstract Aws.Crypto.ImportPrivateRSAKeyOutput
            _ImportPrivateRSAKey(Aws.Crypto.ImportRSAKeyInput input);
    }
}